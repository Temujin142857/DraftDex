...
const Home = (props) => {
  ...
  const [text, setText] = useState({
    ...
    homeHeader2:
      "Select two rosters to begin comparing or create a new roster to add to the draft",
  ...
  return (
    ...
    >
      ...
      <h3 className="text">{text["homeHeader2"]}</h3>
      <ul style={{ columns: "3", marginTop: "5%" }}>
        {rosters &&
          rosters?.map((roster, index) => (
            <li
              className={
                rostersSelected?.includes(roster) ? "lii highlighted" : "lii"
              }
        ...
        {Array.from(Array(emptySlots)).map((_, index) => (
          <li className="lii" key={index}>
            <Link to="/createRoster" className="link">
              {text["newRoster"]}
            </Link>
          </li>
        ))}
